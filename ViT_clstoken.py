...
class WhisperEncoderRegressor(nn.Module):
    ...
    def forward(self, L_input_features, R_input_features, attention_mask=None):
        ...
        # Transformer encoder
        x = self.transformer_encoder(hidden)  # (B, T+1, H)
        x = x[:, 0, :]  # CLS token만 사용 
...
def run_training(train_dataset, test_dataset, L_model_path, R_model_path, save_path, num_epochs=5, batch_size=8, lr=1e-4):
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 5 == 0:
            torch.save(model.state_dict(), os.path.join(save_path, f"model_final_LR_e{epoch+1}.pth"))
    return model
...
def main(cfg: DictConfig):
    print(cfg.model_final_LR_new_root)

    signal_feature = DatasetDict.load_from_disk("/scratch/CPC_data/CPC3/clarity_CPC3_data/whisper/") # TODO : 데이터셋 경로로 변경해야 함 
    ...
    model = run_training(
        ...
        save_path=cfg.model_final_LR_new_root,
        num_epochs=20,     # 5, 20, 100
        batch_size=64,
        lr=1e-5
    )

    torch.save(model.state_dict(), os.path.join(cfg.model_final_LR_new_root, "model_ViT.pth"))
